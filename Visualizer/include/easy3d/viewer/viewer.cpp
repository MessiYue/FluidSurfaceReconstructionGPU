...
//            addModel(model, true, fit);
	...
	void Viewer::draw() {
        ...
        if (program) {
			float pointScale = width() / (camera_->aspectRatio() * tanf(camera_->fieldOfView() * 0.5f));
            program->bind();
			program->set_uniform("MV", MV);
            program->set_uniform("MVP", MVP);
			program->set_uniform("pointScale", pointScale);
            program->set_uniform("wLightPos", wLightPos);
            program->set_uniform("wCamPos", wCamPos);
            program->set_uniform("ssaoEnabled", false);
            ...
            for (auto m : models_) {
                ...
                for (auto d : m->points_drawables()) {
                    if (d->is_visible()) {
						program->set_uniform("pointSize", d->point_size());
                        program->set_uniform("lighting", d->lighting());
                        program->set_uniform("per_vertex_color", d->per_vertex_color() && d->color_buffer());
                        program->set_uniform("default_color", d->default_color());
                        d->draw(false);
                    }
