...
public:
	...
	float distanceSquare(const fVector2& other)const { return fVector2(*this - other).lengthSquare(); }
	float distance(const fVector2& other)const { return sqrt(this->distanceSquare(other)); }
...
inline std::ostream& operator<<(std::ostream& os, const fVector3& a)
{
	os << "(" << a.x << "," << a.y << "," << a.z << ")";
	return os;
}
