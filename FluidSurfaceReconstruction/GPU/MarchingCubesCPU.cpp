	...
	int & triCount)
{
	...
	for (int i = 0; i < 8; i++)
	{
		if (vertices[i].value <= mIsoValue)//位于等值面上或等值面的一侧
			vertexFlag |= 1 << i;
	}
	if (vertexFlag == 0 || vertexFlag == 255)//当前cube所有顶点均在等值面一侧
		return;

	unsigned int edgeFlag = MarchingCubesHelper::edgeFlags[vertexFlag];//低12位用于标记12条边与等值面相交与否
	fVector3 intersectPoss[12];//12条边上的交点位置
	fVector3 intersectNormals[12];//12条边上的交点法向量
	for (int i = 0; i < 12; i++)
	{
		if (edgeFlag & (1 << i))//编号为i的边与等值面相交
		{
...
MarchingCubesCPU::GridVertex MarchingCubesCPU::getVertexData(const iVector3 & index)
{
	...
	//! grid position.
	ret.position.x = mScalarGridInfo.minPos.x + index.x * mScalarGridInfo.cellSize;
	ret.position.y = mScalarGridInfo.minPos.y + index.y * mScalarGridInfo.cellSize;
	ret.position.z = mScalarGridInfo.minPos.z + index.z * mScalarGridInfo.cellSize;
	//! normal.
